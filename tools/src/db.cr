...
class App < Admiral::Command
  class ReadmeWriter < Admiral::Command
    ...
    def run
      frameworks = list_of
      lines = [
        "|    | Language | Framework | Speed (%s) |" % [flags.concurrencies.first],
        "|----|----------|-----------|-----------:|",
      ]
      c = 1
      sorted = frameworks.values.sort do |rank0, rank1|
        rank1[flags.concurrencies.first].to_f <=> rank0[flags.concurrencies.first].to_f
      end
      sorted.each do |row|
        pp row
        lines << "| %s (%s)| [%s](%s) (%s) | %s |" % [
          ...
          row["concurrency_#{flags.concurrencies.first}"].to_f.trunc.format(delimiter: ' ', decimal_places: 0),
        ]
        c += 1
      end
